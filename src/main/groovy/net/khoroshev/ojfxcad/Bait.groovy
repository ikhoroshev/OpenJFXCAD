...
public class Bait{
    ...
    private CSG section(double high, double width){
        ...
        CSG bottomCylinder = (new Cylinder(bottomFilletR, 0.01D, numSlices)).toCSG()
                .transformed(unity().translate(0D, -halfHigh + bottomFilletR, 0));
        CSG rightTopCylinder = (new Cylinder(topFilletR, 0.01D, numSlices)).toCSG()
                .transformed(unity().translate(halfWidth - topFilletR, halfHigh - topFilletR, 0));
    ...
    private CSG tail() {
        ...
        for (int i = 1; i <= tailLengh; i++) {
            ...
            sections.add(
                    section(high*coef,width*coef).transformed(unity().translate(0, 0, i))
            );
        }
        return sections.get(0).hull(sections)
                .transformed(unity().mirror(eu.mihosoft.vvecmath.Plane.XZ_PLANE))
                .transformed(unity().translate(0,0,tailLengh));
    }
    ...
    private CSG head() {
        ...
        return new Sphere(sphereR, numSlices, numSlices).toCSG()
                .transformed(unity().translate(0, 0, sphereR))
                .hull(sections)
                .transformed(unity().mirror(eu.mihosoft.vvecmath.Plane.XZ_PLANE))
                .transformed(unity().translate(0,0,tailLengh));
    }
